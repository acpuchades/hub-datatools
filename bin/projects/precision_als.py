...
def load_hub_episode_diagnoses(datadir: Path, name: str, episodes: DataFrame) -> DataFrame:
	return load_data(datadir, name).merge(episodes, left_on='episode_id', right_index=True)
...
class PrecisionALS(Project):
	...
	def describe(self) -> None:
		gene_info = self._patients[['c9_status', 'sod1_status', 'atxn2_status']]
		...
		print(f'\t * 1+ follow-up -> {len(self._als_data.value_counts("pid"))}')
		print(f'\t * 2+ follow-up -> {sum(self._als_data.value_counts("pid") >= 2)}')
		print()

		print(' Biogen Extant Task 3')
		print(' --------------------')
		print(f' > Time to ambulation support: {len(self._als_data[self._als_data.caminar <= 2].value_counts("pid"))}')
		print(f' > Time to CPAP: {len(self._resp_data[self._resp_data.cpap.fillna(False)].value_counts("pid"))}')
		print(f' > Time to VMNI: {len(self._resp_data[self._resp_data.portador_vmni.fillna(False)].value_counts("pid"))}')
		print(f' > Time to PEG: {len(self._nutr_data[self._nutr_data.indicacion_peg.fillna(False)].value_counts("pid"))}')

		print(f' > Time to MiToS: {len(self._als_data[self._als_data.mitos.notna()].value_counts("pid"))}')
		for n in range(1, 5):
			print(f'\t * Stage {n} -> {len(self._als_data[self._als_data.mitos == n].value_counts("pid"))}')

		print(f' > Time to King\'s: {len(self._als_data[self._als_data.kings.notna()].value_counts("pid"))}')
		for n in range(1, 5):
			print(f'\t * Stage {n} -> {len(self._als_data[self._als_data.kings == n].value_counts("pid"))}')
	...
	def export_data(self) -> DataFrame:
		...
		return DataFrame({
			...
			'c9_status': self._patients.c9_status.replace(GENE_STATUS_CATEGORIES),
			'sod1_status': self._patients.sod1_status.replace(GENE_STATUS_CATEGORIES),
			'fus_status': None,
			'tardbp_status': None,
			...
			'kings_1': self._als_data[self._als_data.kings == 1].groupby('pid').fecha_visita.min(),
			'kings_2': self._als_data[self._als_data.kings == 2].groupby('pid').fecha_visita.min(),
			'kings_3': self._als_data[self._als_data.kings == 3].groupby('pid').fecha_visita.min(),
			'kings_4': self._als_data[self._als_data.kings == 4].groupby('pid').fecha_visita.min(),

			'mitos_1': self._als_data[self._als_data.mitos == 1].groupby('pid').fecha_visita.min(),
			'mitos_2': self._als_data[self._als_data.mitos == 2].groupby('pid').fecha_visita.min(),
			'mitos_3': self._als_data[self._als_data.mitos == 3].groupby('pid').fecha_visita.min(),
			'mitos_4': self._als_data[self._als_data.mitos == 4].groupby('pid').fecha_visita.min(),

...
def _load_als_data_sql(con: Connection) -> DataFrame:
	...
	als_data.rename(columns={'pid': 'id_paciente'}, inplace=True)
...
def _load_resp_data_sql(con: Connection) -> DataFrame:
	...
	resp_data.rename(columns={'pid': 'id_paciente'}, inplace=True)
...
def _load_nutr_data_sql(con: Connection) -> DataFrame:
	...
	nutr_data.rename(columns={'pid': 'id_paciente'}, inplace=True)
...
def _add_patient_genetic_data(df: DataFrame) -> None:
	...
	apply_transform_pipeline(df, 'resultado_estudio_c9', OPT_ENUM_PIPELINE, inplace='estado_c9')
	apply_transform_pipeline(df, 'resultado_estudio_sod1', OPT_ENUM_PIPELINE, inplace='estado_sod1')
...
def _clean_als_data(df: DataFrame) -> None:
	...
	apply_transform_pipeline(df, 'insuficiencia_respiratoria', OPT_NUMBER_PIPELINE, inplace='insuf_resp')
